...
class CosyVoiceModel:
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid, embedding):
        ...
        inputs_stage1 = {
            "text": text.numpy(),
            "text_len": text_len,
            "prompt_text": prompt_text.numpy(),
            "prompt_text_len": prompt_text_len,
            "prompt_speech_token": llm_prompt_speech_token.numpy(),
            "prompt_speech_token_len": np.array([llm_prompt_speech_token.shape[1]], dtype=np.int32),
            "embedding": llm_embedding.numpy()
        }

        # 获取 Stage 1 的输入名称并运行推理
        input_names_stage1 = [input.name for input in self.llm_stage1_session.get_inputs()]
        lm_input, text_len_output = self.llm_stage1_session.run(None, {
            input_names_stage1[0]: inputs_stage1["text"],
            input_names_stage1[1]: inputs_stage1["text_len"],
            ...
            input_names_stage1[5]: inputs_stage1["prompt_speech_token_len"],
            input_names_stage1[6]: inputs_stage1["embedding"]
        })

        # 初始化缓存
        att_cache = np.zeros((0, 0, 0, 0), dtype=np.float32)  
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, uuid, finalize=False):
        ...
        inputs = {
            ...
            'prompt_token_len': np.array([prompt_token.shape[0]], dtype=np.int32), 
    ...
    def inference(self, text, flow_embedding, llm_embedding=torch.zeros(0, 192),
        ...
        if self.tts_speech_token_dict[this_uuid]:
            ...
            # 使用预定义的 token 大小作为返回的默认值
            this_tts_speech_token = torch.zeros(1, 0, self.expected_token_size, device=self.device)
