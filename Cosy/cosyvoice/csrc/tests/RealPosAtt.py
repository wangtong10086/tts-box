...
class RelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def rel_shift(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # (batch, head, 2*time1-1, time1) --> (batch, head, time1, time1)
    ...
    def infer(
        ...
        context_len = key_cache.shape[0]

        num_blocks_per_seq = (context_len + block_size - 1) // block_size # 计算块数 (num_blocks_per_seq)，用于确定如何将上下文长度划分为多个块。
...
def test_att():
    ...
    pos_emb = torch.rand(batch_size, time2, dim) # Positional embedding tensor
    cache = torch.rand((1, head_num, cache_t, head_size*2)) # Cache tensor

    # 调用注意力层的 forward 方法
    output, new_cache = attention_layer(query, query, query, mask, pos_emb, cache)

    # 打印输出和新缓存的形状
    print("Output shape:", output.shape)           # 预期形状: (batch_size, time1, n_feat)
    print("New cache shape:", new_cache.shape)      # 预期形状: (1, n_head, time2 + 0, n_feat * 2)
