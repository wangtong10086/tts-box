...
void copy_blocks(
  ...
  int64_t* key_cache_ptrs;
  int64_t* value_cache_ptrs;
  cudaMalloc((void**)&key_cache_ptrs, num_layers * sizeof(int64_t));
  cudaMalloc((void**)&value_cache_ptrs, num_layers * sizeof(int64_t));

  // Create an array of pointers to the key and value caches.
  std::vector<int64_t> key_cache_ptrs_host(num_layers);
  std::vector<int64_t> value_cache_ptrs_host(num_layers);
  for (int layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    key_cache_ptrs_host[layer_idx] = reinterpret_cast<int64_t>(key_caches[layer_idx].data_ptr());
    value_cache_ptrs_host[layer_idx] = reinterpret_cast<int64_t>(value_caches[layer_idx].data_ptr());
  }

  // Copy pointers to device
  cudaMemcpy(key_cache_ptrs, key_cache_ptrs_host.data(), num_layers * sizeof(int64_t), cudaMemcpyHostToDevice);
  cudaMemcpy(value_cache_ptrs, value_cache_ptrs_host.data(), num_layers * sizeof(int64_t), cudaMemcpyHostToDevice);
  ...
  // Move the block mapping array to the GPU.
  ...
  // Free device memory
  cudaFree(key_cache_ptrs);
  cudaFree(value_cache_ptrs);
}
