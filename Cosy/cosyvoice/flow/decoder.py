...
class ConditionalDecoder(nn.Module):
    ...
    def forward(self, x, mask, mu, t, spks=None, cond=None):
        ...
        for resnet, transformer_blocks, upsample in self.up_blocks:
            ...
            # 将 x 的时间维度裁剪为 skip 的时间维度
            x_cropped = x[:, :, :skip.shape[-1]]  # 形状变为 (b, c, skip_time_steps)
            # 在指定维度上拼接 x_cropped 和 skip，通常在 channel 维度 (dim=1)
            x = torch.cat((x_cropped, skip), dim=1)  # 拼接后的形状为 (b, c + num_channels_skip, skip_time_steps)
